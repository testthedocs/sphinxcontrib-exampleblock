...
CSS_FILE = 'exampleblock.css'
JS_FILE = 'exampleblock.js'


class ExampleBlockDirective(Directive):
    """
    This directive is intended to be used to contain a group of
    example blocks which are beingused to show code examples in many different
    ...
    def run(self):
        ...
        node['classes'].append('example-block')
        self.add_name(node)
...
def copy_assets(app, exception):
    if app.builder.name != 'html' or exception:
        return
    app.info('Copying exampleblock stylesheet/javascript... ', nonl=True)
...
def setup(app):
    app.add_directive('example-block',  ExampleBlockDirective)
